...
ape_template = '''
Short 3 bullet points about additional information to help justify there is no violation in the following scenario.
----
Scenario: {{scenario}}
'''.strip('\n')
...
for name in names:
    ...
    for idx, violation in tqdm(enumerate(violations)):
        ...
        #  content = ri_query.render(dict(persona=persona, scenario=violation['input']))
        #  response = completion(
            #  model=QWEN3,
            #  messages=[dict(role='user', content=content)],
    ...
    #  Dataset.from_list(ds).to_json('output/ri_jb.jsonl')
